    ...
    def _install_request_loader(self):
        ...
        def oauth2_load_user_from_request(request):
            ...
            if apikey:
                ...
                # CKAN-native API tokens (api_token_create / legacy API key) are
                # validated locally against the site secret. Resolve them first
                # so they never trigger an external OAuth2 profile API round-trip.
                native_user = _get_user_for_apitoken()
    ...
    def identify(self):
        ...
        if apikey:
            ...
            if apikey.startswith("Bearer "):
                apikey = apikey[7:].strip()
            # CKAN-native API tokens (api_token_create / legacy API key) are
            # validated locally against the site secret. Resolve them first so
            # they never trigger an external OAuth2 profile API round-trip.
            native_user = _get_user_for_apitoken()
            if native_user is not None:
                user_name = native_user.name
                user_obj = native_user
            else:
                try:
                    token = {"access_token": apikey}
